...
export default function OrderComplete(props){
    ...
    const [orderNumber, setNumber] = useState(null)
    const [searchParams] = useSearchParams();
    const cart = localStorage.getItem('cart');

    useEffect(() => {
        ReactGA.pageview(window.location.pathname);
        axios.post("https://skylineculture-api.onrender.com/order-complete", {
            session_id: searchParams.get("session_id"),
            cart: localStorage.getItem('cart')
        }).then((res) => {
            setName(res.data.name.split(' ')[0]);
            setNumber(res.data.orderNumber);
        });

        axios.post('https://skylineculture-api.onrender.com/remove-inventory', {
